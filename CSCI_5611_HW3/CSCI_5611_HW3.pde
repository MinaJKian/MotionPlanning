...
void setup() {
  ...
  //agents.add(new Agent(new PVector(mult, height-mult), new PVector(width-1*mult,height-1*mult), agentRad));
  agents.add(new Agent(new PVector(mult, mult), new PVector(mult+50, mult), agentRad, new PVector(10,0,0)));
  agents.add(new Agent(new PVector(mult+50, mult), new PVector(width-1*mult,1*mult), agentRad, new PVector(-10,0,0)));
    ...
    while (q.size() != 0) {
      ...
      for (int neighborIndex : neighbors) { 
        ...
        if (explored.indexOf(neighborIndex) == -1 && !inQ) { // if it's not disconnected
          ...
          q.add(new Node(points.get(neighborIndex),newCost+heuristic(end,points.get(neighborIndex)),neighborIndex)); // add to neighbors list (may have already been checked though!
...
class Agent {
  ...
  Agent(PVector pos, PVector goal, float rad, PVector goalVel) {
    this.pos = pos;
    this.rad = rad;
    this.goal = goal;
  ...
  // if we haven't already reached the goal, then move 
    ...
    if (state != path.size()) {
      ...
      PVector goalPos = path.get(state);
      ...
      goalVel = PVector.mult(dirNorm, dt*100);
      
      PVector oldPos = pos;
      pos = PVector.add(pos, goalVel, null);
      
      // if the distance between where I am and where I started is larger than the distance between
      // where I am and where I want to be, update state!
      float curDist = PVector.dist(pos, oldPos);
      float desDist = PVector.dist(goalPos,oldPos);
    
      // if we are at or past the point we want to be at, turn to the next point!!!
      // this probably works but honestly can't even tell if I don't readjust it sooo 
      if (curDist >= desDist) {
        state++;
        // gets rid of fuzziness when moving obstacles when agent at goal
      }
      if (state == path.size()) {
...
// TODO: deal with moving obstacles. just a click is also a press, so maybe don't create a circle right away?
 //maybe have different modes?
void mouseClicked() {
  println("just clicked"); 
}

// TODO: decide whether or not it's worth it to add the obstacle to the array right away
void mousePressed() {
...
void mouseDragged() {
  ...
  if (mode == 0) {
    ...
    currentObstacle.updateObstacleRadius(rad);
    prm = new PRM();
...
void mouseReleased() {
  ...
  if (mode == 0) {
    ...
    agents.add(new Agent(clickPos, currentPos, agentRad, new PVector(0,0,0))); 
    Forces = new PVector[agents.size()];
  }
